...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initState() {
    ...
    _tweetsFuture = Future.value(<Tweet>[]);
    _initializeSession();
  }
  ...
  Future<void> _createTweet() async {
    if (!_isAuthenticated()) {
      _showAuthRequiredDialog('crear tweets');
      return;
    }
  ...
  Future<void> _deleteTweet(int id) async {
    if (!_isAuthenticated()) {
      _showAuthRequiredDialog('borrar tweets');
      return;
    }
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text('Debes iniciar sesion para $action.'),
          actions: [
            TextButton(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: _currentUser != null
                ? _logout
                : () => Navigator.of(context).pushReplacementNamed('/login'),
            tooltip: _currentUser != null ? 'Cerrar sesion' : 'Iniciar sesion',
          ),
        ],
      ),
      body: Column(
        ...
        children: [
          ...
          if (!_isAuthenticated())
            const Padding(
              ...
              child: Text(
                'Inicia sesion para publicar o borrar tweets',
  ...
  Widget _buildTweetCard(Tweet tweet) {
    final canDelete = _currentUser != null && tweet.postedByUsername == _currentUser!.username;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Publicado por: ${tweet.postedByUsername ?? 'Desconocido'}',
